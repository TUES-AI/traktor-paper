...
exec "$PYTHON_BIN" embedded/scripts/train_real_predictive_sac.py \
  ...
  --motion-novelty-weight "${PCVM_THETA_FRONT_MOTION_NOVELTY_WEIGHT:-0.80}" \
  --new-cluster-bonus "${PCVM_THETA_FRONT_NEW_CLUSTER_BONUS:-1.20}" \
  --slow-surprise-weight "${PCVM_THETA_FRONT_SURPRISE_WEIGHT:-0.05}" \
  --safe-motion-bonus "${PCVM_THETA_FRONT_SAFE_MOTION_BONUS:-0.50}" \
  --safe-motion-min-cm "${PCVM_THETA_FRONT_SAFE_MOTION_MIN_CM:-10}" \
  --executed-distance-weight "${PCVM_THETA_FRONT_EXECUTED_DISTANCE_WEIGHT:-0.70}" \
  --recent-revisit-penalty "${PCVM_THETA_FRONT_RECENT_REVISIT_PENALTY:-0.10}" \
  ...
  --slow-recovery-penalty "${PCVM_THETA_FRONT_RECOVERY_PENALTY:-0.20}" \
  --zero-forward-cm "${PCVM_THETA_FRONT_ZERO_FORWARD_CM:-3}" \
  --zero-forward-penalty "${PCVM_THETA_FRONT_ZERO_FORWARD_PENALTY:-0.35}" \
  --loop-memory-size "${PCVM_THETA_FRONT_LOOP_MEMORY_SIZE:-25}" \
  --loop-near-radius-m "${PCVM_THETA_FRONT_LOOP_NEAR_RADIUS_M:-0.45}" \
  --loop-revisit-penalty "${PCVM_THETA_FRONT_LOOP_REVISIT_PENALTY:-0.45}" \
  ...
  --recovery-streak-penalty "${PCVM_THETA_FRONT_RECOVERY_STREAK_PENALTY:-0.10}" \
