"""PCVM-D: PCVM with a frozen DINOv2 visual encoder.

This backend keeps the same observation/reward contract as PCVM/PCVM-M but
replaces the rover CNN/MobileNet visual feature extractor with DINOv2 features.
It is intended as a quick self-supervised visual-memory diagnostic:
does a stronger frozen image encoder produce more useful visual memory banks?
"""
...
class DINOv2VisualEncoder(nn.Module):
    def __init__(self, model_name='facebook/dinov2-small', out_dim=256):
        ...
        self.proj = nn.Sequential(nn.Linear(hidden_size, out_dim), nn.ReLU(), nn.Linear(out_dim, out_dim), nn.ReLU())
...
class PCVMjNet(nn.Module):
    def __init__(self, action_dim=PCVM_DEFAULT_ACTION_DIM, model_name='facebook/dinov2-small'):
        super().__init__()
        self.action_dim = int(action_dim)
        self.visual = DINOv2VisualEncoder(model_name=model_name, out_dim=256)
...
class PCVMDINO:
    def __init__(self, device=None, action_dim=PCVM_DEFAULT_ACTION_DIM, model_name='facebook/dinov2-small'):
        self.device = torch.device(device or ('cuda' if torch.cuda.is_available() else 'cpu'))
        self.action_dim = int(action_dim)
        self.model_name = model_name
        self.net = PCVMjNet(action_dim=self.action_dim, model_name=model_name).to(self.device)
    ...
    def observe(self, frame_bgr, sensors, motion, action, dt):
        ...
        return {
            ...
            'pcvm_visual': f'dinov2:{self.model_name}',
            'step': self.step,
        }
