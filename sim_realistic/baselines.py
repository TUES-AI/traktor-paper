...
class DeterministicExplorer:
    """Reactive local-only baseline over [theta_norm, distance_norm]."""
    ...
    def act(self, obs: np.ndarray) -> np.ndarray:
        ...
        if self.turn_timer > 0:
            self.turn_timer -= 1
            return np.array([self.turn_dir, -1.0], dtype=np.float32)
        if front < 0.20:
        ...
        if self.rng.random() < 0.035:
            return np.array([float(self.rng.uniform(-0.7, 0.7)), 0.15], dtype=np.float32)
