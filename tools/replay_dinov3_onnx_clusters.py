#!/usr/bin/env python3
"""Offline DINOv3-ONNX image clustering experiment.

This script is intentionally outside the runtime rover policy. It replays saved
image folders, extracts frozen DINOv3 ViT-S/16 pooler embeddings with ONNX
Runtime, clusters only those image embeddings with a small memory-bank rule, and
writes a cluster montage PNG plus a JSONL log. DINO features must not be fed to
the sensory GRU/SAC stack; they are only for paper analysis or future reward-only
ablations.
"""
...
def main():
    ...
    parser.add_argument('--frame-dir', required=True, help='Directory of saved JPG frames to cluster offline')
